...
Triangle::Triangle(Screen *screen) : Shape(screen)
{
    m_fill = false;
}
...
void Triangle::setCoordinates(Point p, ...)
{
    ...
    va_list ap;
    va_start(ap, p);
    m_vertex.push_back(va_arg(ap, Point));
    m_vertex.push_back(va_arg(ap, Point));
    va_end(ap);
...
void Triangle::copy(const Triangle &t)
{
    ...
    if (t.m_vertex.size() > 0)
    {
        m_vertex.push_back(t.m_vertex.at(0));
        m_vertex.push_back(t.m_vertex.at(1));
        m_vertex.push_back(t.m_vertex.at(2));
    }
}
